 ...
 */
async showFeedback(message: string, feedbackType: FeedbackType, offsetX: number | null) : Promise<null> {
    return await TAURI_INVOKE("show_feedback", { message, feedbackType, offsetX });
},
/**
 * 隐藏反馈窗口
 */
...
export type AudioFlowPanelPositionChanged = { position: AudioFlowPanelPosition }
export type AudioState = { type: "idle" } | { type: "dictating"; mode: DictatingMode } | { type: "transcribing" }
...
export type SettingsStore = { theme?: AppTheme; audioFlowPanelPosition?: AudioFlowPanelPosition; llmApiKey?: string | null; llmSystemPrompt?: string | null; autostartEnabled?: boolean }
export type ShowFeedback = { message: string; type: FeedbackType }
export type TranscriptionResult = { text: string; durationMs: number | null; utterances: TranscriptionUtterance[] }
